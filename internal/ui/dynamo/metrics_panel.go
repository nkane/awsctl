...
// dynamoMetricSpecs returns the CloudWatch metrics shown for a DynamoDB table.
func dynamoMetricSpecs(table string) []metricSpec {
	dim := map[string]string{"TableName": table}
	return []metricSpec{
		{Title: "Read Capacity (sum)", Namespace: "AWS/DynamoDB", MetricName: "ConsumedReadCapacityUnits", Dimensions: dim, Stat: "Sum"},
		{Title: "Write Capacity (sum)", Namespace: "AWS/DynamoDB", MetricName: "ConsumedWriteCapacityUnits", Dimensions: dim, Stat: "Sum"},
		{Title: "Throttled Requests", Namespace: "AWS/DynamoDB", MetricName: "ThrottledRequests", Dimensions: dim, Stat: "Sum"},
	}
}
