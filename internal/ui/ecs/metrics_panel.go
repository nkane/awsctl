...
// ecsMetricSpecs returns the CloudWatch metrics shown for an ECS service.
func ecsMetricSpecs(cluster, service string) []metricSpec {
	dim := map[string]string{"ClusterName": cluster, "ServiceName": service}
	return []metricSpec{
		{Title: "CPU Utilization (avg %)", Namespace: "AWS/ECS", MetricName: "CPUUtilization", Dimensions: dim, Stat: "Average"},
		{Title: "Memory Utilization (avg %)", Namespace: "AWS/ECS", MetricName: "MemoryUtilization", Dimensions: dim, Stat: "Average"},
	}
}
